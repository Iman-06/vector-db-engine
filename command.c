// command.c — Command parsing, dispatch, and response formatting.0
...
static void cmd_add(int fd, const server_config_t *cfg, char *rest) {
    ...
    char *saveptr;// it is used to keep track of the current position in the string for subsequent calls to strtok_r.
    char  *endp;
    ...
    for (int i = 0; i < dim; i++) {
        tok = strtok_r(NULL, " \t\r\n", &saveptr);// subsequent calls to strtok_r with NULL continue tokenizing the same string. It returns the next token or NULL if there are no more tokens.
        if (!tok) {
...
static void cmd_search(int fd, const server_config_t *cfg, char *rest) 
{
    ...
    if (rc == VS_OK && out_count > 0) {
        snap = malloc((size_t)out_count * (size_t)dim * sizeof(float));// this is the snapshot buffer for the result vectors. It is a flat array where each result's vector components are stored contiguously
            for (int i = 0; i < out_count; i++) {
                const float *v = vs_get_vector(cfg->store, results[i].store_index);
                if (v)
                {memcpy(snap + (size_t)i * (size_t)dim, v, (size_t)dim * sizeof(float));}
            }
    }
    ...
    for (int i = 0; i < out_count; i++) {
        ...
        pos = snprintf(line, sizeof(line), "%lld %.6f", (long long)results[i].id,results[i].distance);
        ...
        if (snap) {
            ...
            for (int j = 0; j < dim; j++) {
                ...
                pos += snprintf(line + pos, sizeof(line) - (size_t)pos, " %.6f", v[j]);
            }
        }
...
void handle_client(int fd, const server_config_t *cfg) {
    ...
    FILE *in = fdopen(rfd, "r"); // coverts socket into stream file
    if (!in) {
