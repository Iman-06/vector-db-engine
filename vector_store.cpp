...
static hm_entry_t* hm_alloc_buckets(size_t cap) {
    hm_entry_t* b = static_cast<hm_entry_t*>(calloc(cap, sizeof(hm_entry_t)));
    return b;
}
...
static int vs_grow(vector_store_t* vs, size_t needed) {
    ...
    float* nv = static_cast<float*>(realloc(vs->vectors, newcap * static_cast<size_t>(vs->dim) * sizeof(float)));
    int64_t* ni = static_cast<int64_t*>(realloc(vs->ids, newcap * sizeof(int64_t)));
...
int vs_init(vector_store_t* vs, int dim) {
    ...
    if (pthread_mutex_init(&vs->lock, nullptr) != 0) {
        hm_destroy(&vs->map);
        free(vs->vectors);
        free(vs->ids);
        return VS_ERR_NOMEM;
...
int vs_add(vector_store_t* vs, int64_t id, const float* comps) {
    ...
    if (exists) {
        memcpy(vs->vectors + idx * static_cast<size_t>(vs->dim),
            comps,
            static_cast<size_t>(vs->dim) * sizeof(float));
...
const float* vs_get_vector(const vector_store_t* vs, size_t i) {
    if (i >= vs->count) return nullptr;
    return vs->vectors + i * static_cast<size_t>(vs->dim);
}
